...
*/
	...
	int      i=0, ret=0;            /*loop variables, function return value */
	...
	/****************************************************************************/
		...
		for(i=1;i<=FlowParam.active_count;i++) {
			...
			//Add flow elevation to the DEM (add to dem_elev in case elev_uncert)
			dataGrid[CAList[i].row][CAList[i].col].elev = 
				dataGrid[CAList[i].row][CAList[i].col].dem_elev = 
				CAList[i].elev;
		}
