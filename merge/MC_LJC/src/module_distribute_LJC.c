...
*/
	...
	unsigned n, active, NCol,NRow;      /*neighbor counter, grid location               */
	...
	do { 
		...
		if((CAList+c)->thickness > 0) {
			...
			neighborCAList = NEIGHBOR_ID((CAList+c),    /* Automata Center Cell  */   
			 /* neighborCAList = NEIGHBOR_ID(CAList[c], */
			...
			if (neighborCount > 0) {
				...
				for(n=0; n<neighborCount;n++) {
					...
					active = dataGrid[NRow][NCol].active;
					if(!active){
						...
						*activeCount = ACTIVATE(dataGrid,      /*DataCell Global Data Grid */
						...
						if(*activeCount < 0){
							printf("\nError [DISTRIBUTE]: [ACTIVATE] Active count < 0!\n");
							return 1;
						}
					...
					(CAList+active)->thickness +=
					       volumeDistribute * neighborEffRelief / totalRelief;
					(CAList+active)->elev      +=
					       volumeDistribute * neighborEffRelief / totalRelief;
					/*fprintf (stderr, "[%d][row=%d][col=%d] thickness=%f elev=%f\n", 
					         (int) active, 
					         (int) NRow,
					         (int) NCol,
					         (CAList+active)->thickness,
			...
			/*Check for Flow off map (neighborCount returns negative value)*/
			else if(neighborCount < 0) {
				printf("ERROR [DISTRIBUTE]:  [NEIGHBOR_ID] Flow off map!\n");
